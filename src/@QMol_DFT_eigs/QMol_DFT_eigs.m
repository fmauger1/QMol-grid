classdef QMol_DFT_eigs < QMol_suite
%QMol_DFT_eigs eigen-state solver for density-functional theory (DFT) level
%   of theory.
%
%   NOTES:
%     > QMol_DFT_eigs uses a private random number stream to ensure 
%       reproducibility across runs
%     > QMol_DFT_eigs imposes eigen state to be real valued
...
methods (Static,Access={?QMol_doc,?QMol_DFT_eigs})
function showInfo
    fprintf('  * QMol_DFT_eigs:\n');
    fprintf('      > DFT eigen-state solver\n      > MATLAB eigs function\n'); 
    QMol_DFT_eigs.version;
end
end
...
%   values
    ...
    % General options
    fprintf('  * Eigen-state solver for DFT Hamiltonians           MATLAB eigs function\n')
    if ~obj.issym
...
%clear clears all or selected member properties
    ...
    if isempty(obj.disp),   obj.disp    =   false;  end
end
end
methods (Access=public)
