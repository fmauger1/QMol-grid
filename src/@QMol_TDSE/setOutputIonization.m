...
case {'init','initialize','initialization'} %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if obj.sIon
        ...
        if ischar(obj.sIWfcnI),                     if strcmpi(obj.sIWfcnI,'all')       % Save all wave functions
            % What to save
            obj.oIon.indexWaveFunction  =   1:obj.SE.N;
            
            % Allocation
            obj.oIon.waveFunction   =   NaN(obj.SE.N,numel(obj.oIon.time));
                                                    else
            warning('QMol:TDSE:outputIonization',...
        ...
        elseif ~isempty(obj.sIWfcnI)                                                      % User-supplied indexes
            ...
            % What to save
            obj.oIon.indexWaveFunction  =   1:numel(obj.SE.N);
            
            % Allocation
            obj.oIon.waveFunction   =   NaN(numel(obj.SE.N),numel(obj.oIon.time));
        end
    else
        obj.oIon.ind    =   NaN;
        obj.oIon.n      =   1;
...
otherwise %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Unexpected option
    error('QMol:TDSE:setOutputIonization',['Unknown option ' opt]);
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
end