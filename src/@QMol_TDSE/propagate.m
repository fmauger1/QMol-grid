...
%propagate propagates the TDSE equations using the (child object) scheme.
    ...
    if isa(opt,'QMol_SEq')                                                  % TDSE propagation from scratch
        % Initialize the DFT object
        if obj.disp,    QMol_doc.showSection('Build the Schrodinger-equation (SE) model'); %#ok<ALIGN> 
                        opt.initialize(2);
    ...
    elseif  ischar(opt) && any(strcmpi(opt,{'restart','resume'}))           % Restart mode
        % Initialize the SE object
        if obj.disp,    QMol_doc.showSection('Build Schrodinger-equation (SE) model'); %#ok<ALIGN> 
                        obj.SE.initialize(2);
        else,           obj.SE.initialize(0);                               end
        if obj.sAcc,    obj.SE.V.setDerivative;                             end
        ...
        error('QMol:TDSE:propagate','Unexpected TDSE propagation case');
    end
    ...
    for k = kstart:numel(obj.tspan)-2
        ...
        if ~isempty(obj.ABC),       obj.ABC.applyMask(obj.SE.wfcn);         end
    ...
    % Finalization ========================================================
        ...
        if obj.disp
            ...
            fprintf('    TDSE object and specified output MATLAB file(s).\n\n');
