    ...
    if obj.QM.isSpinPol     % Spin polarized
        ...
        if nargin < 4, if isempty(KSO)                                      %#ok<ALIGN> 
            ...
            KSO         =   QMol_DFT_orbital('isSpinPol',true,      ...
                                'KSOup',zeros(numel(obj.x),N(1)),   ...
                                'KSOdw',zeros(numel(obj.x),N(2)));
        elseif ~KSO.isSpinPol || any(numel(obj.x) ~= [size(KSO.KSOup,1) size(KSO.KSOdw,1)])
            % Different spin polarization or incompatible sizes
            KSO.set('isSpinPol',true,'KSO',[],                      ...
                    'KSOup',zeros(numel(obj.x),N(1)),'KSOdw',zeros(numel(obj.x),N(2)));
        else
            % Initialization
            SU          =   size(KSO.KSOup);
            SD          =   size(KSO.KSOdw);
            ...
            else,               KSO.KSOup   =  [KSO.KSOup zeros(numel(obj.x),N(1)-SU(2))]; end

            if SD(2) > N(2),    KSO.KSOdw   =   KSO.KSOdw(:,1:N(2));
            else,               KSO.KSOdw   =  [KSO.KSOdw zeros(numel(obj.x),N(2)-SD(2))]; end, end

        % Allocate random orbital ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        else, if isempty(KSO),  KSO         =   QMol_DFT_orbital();                                     end
            if isR,             KSO.set('isSpinPol',true,'KSO',[],          ...
                                        'KSOup',rand(randStr,[numel(obj.x),N(1)])-.5,                   ...
                                        'KSOdw',rand(randStr,[numel(obj.x),N(2)])-.5);                  %#ok<ALIGN> 
            else,               KSO.set('isSpinPol',true,'KSO',[],          ...
                                        'KSOup',rand(randStr,[numel(obj.x),N(1)],'like',1i)-.5-.5i,     ...
                                        'KSOdw',rand(randStr,[numel(obj.x),N(2)],'like',1i)-.5-.5i);    end
            % Normalize orbitals
        ...
        if nargin < 4, if isempty(KSO)                                      %#ok<ALIGN> 
            % Create (spin-polarized) object from scratch
            KSO         =   QMol_DFT_orbital('isSpinPol',false,'KSO',zeros(numel(obj.x),N));

        elseif KSO.isSpinPol || numel(obj.x) ~= size(KSO.KSO,1)
            % Different spin polarization or incompatible sizes
            KSO.set('isSpinPol',false,'KSO',zeros(numel(obj.x),N),'KSOup',[],'KSOdw',[]);
        else
            % Initialization
            S           =   size(KSO.KSO);
            ...
            else,               KSO.KSO     =  [KSO.KSO zeros(numel(obj.x),N(1)-S(2))]; end, end

        % Allocate random orbital ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        else, if isempty(KSO),  KSO         =   QMol_DFT_orbital();                             end
            if isR,             KSO.set('isSpinPol',false,'KSOup',[],'KSOdw',[],                ...
                                        'KSO',rand(randStr,[numel(obj.x),N])-.5);               %#ok<ALIGN> 
            else,               KSO.set('isSpinPol',false,'KSOup',[],'KSOdw',[],                ...
                                        'KSO',rand(randStr,[numel(obj.x),N],'like',1i)-.5-.5i); end
            % Normalize orbitals
