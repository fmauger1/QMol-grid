classdef QMol_DFT_profiler < QMol_suite
%QMol_DFT_profiler memory and/or execution time profiling for DFT model or
%   functional
...
methods (Static,Access={?QMol_doc,?QMol_DFT_profiler})
function showInfo
...
if any(strcmpi(obj.mode,{'all','memory','size'}))
    ...
    fprintf('                                                                ----------\n');
    fprintf('                                                        TOTAL =  ');
    QMol_DFT_profiler.showMemoryFormatted(mem);
    fprintf('\n\n');
...
%   of the type opt. The memory footpring m is expressed in bites
    ...
    switch lower(opt)
        ...
        case {'imag','complex'}
            m           =   16 * N;
        otherwise
            warning('QMol:DFT_profiler:arrayType',['Unknown array type: ' opt '. No memory evaluation performed']);
            m           =   0;
    end
...
%showMemoryFootprint displays the memory foortrint 
    ...
    switch lvl
        case 1
            fprintf('  * %-60s ',msg);      QMol_DFT_profiler.showMemoryFormatted(mem);
        case 2
            fprintf('    > %-58s ',msg);    QMol_DFT_profiler.showMemoryFormatted(mem);
        otherwise
            fprintf('    > %-59s ',msg);    QMol_DFT_profiler.showMemoryFormatted(mem);
            warning('QMol:DFT_profiler:showLevel','Maximum show level is 2; Entry downgraded to level 2');
    end
    fprintf('\n');
end
end
