classdef QMol_TDDFT_SSO_2S < QMol_TDDFT_SSO
%QMol_TDDFT_SSO_2S symplectic split-operator TDDFT propagator using a
...
%   for the Strang splitting propagator
    ...
    if obj.isVTV,   obj.setExpV(.5);    if obj.FG ~= 2, obj.setExpT(1 );    end %#ok<ALIGN> 
    else,           obj.setExpT(.5);
                    obj.DFT.Vks =   obj.DFT.getPotential([],obj.DFT.Vks);   end % Initialize V_KS

    if ~isempty(obj.DFT.Vks.Vimp)
        % Implicit potential component
        error('QMol:TDDFT:SSOStrangSplitting:implicitPotential', ...
            'The Strang-splitting propagator does not support implicit Kohn-Sham potentials');
    end
